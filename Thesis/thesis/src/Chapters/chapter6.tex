\chapter{Evaluation}
    ...
    \section{Attack Performance}
        ...
        \subsection*{Attack 2}
            ...
            With rising amount of known edges, the attack performance grows, leading to an \TODO{improvement} up to $0.06$ F1-Score, while comparing the baseline performance with the results of attacks with known edges.
        ...
        \subsection*{Attack 3}
            ...
            The following Figures \refeq{figure:eval-att3-citeseer-citeseer} and \refeq{figure:eval-att3-pubmed-citeseer} present the results for our link stealing attacks on a target model, that was trained on the CiteSeer dataset while the adversary was trained on another distribution dataset.
            The charts for Cora and Pubmed being the target datasets, can be found here. \TODO{update}

            \begin{figure}[h]
                \begin{center}
                    \includegraphics[width=\textwidth]{attack-3-cora-citeseer}
                    \caption{Attack 3 - $D_{f_t} = CiteSeer$ and $D_A = Cora$}
                    \label{figure:eval-att3-citeseer-citeseer}
                \end{center}
            \end{figure}
            ...
            Evaluating \emph{Attack 3}, we note, that the average attack performance of our baseline (\TODO{$0.7492$} F1-Score) is higher than the average baseline performance of \emph{Attack 1} but lower than the average baseline performance of \emph{Attack 2}.
            The results differ, based on the used datasets $D_{f_t}$ and $D_A$ and the architecture of the target model.
            We achieve a minimum baseline attack performance of \TODO{$0.6561$} F1-Score with $D_{f_t}$ being the Cora dataset, the attack model being trained on Pubmed and the target model being a graph convolutional neural network.
            However, when our target model is a graph attention network, which was trained on the CiteSeer dataset, while we set $D_A = Pubmed$, we can achieve a maximum baseline attack performance of \TODO{$0.8363$} F1-Score.  
            Like in \emph{Attack 2}, the attack performance increases as well, while the amount of known edges rises.
            This leads to an improvement up to \TODO{$0.1292$} F1-Score.
            With respect to all performed attacks, the average F1-Score is \TODO{$0.7934$}, with the lowest being \TODO{$0.6561$ (GCN trained on Cora and adversary trained on Pubmed with 0\% known edges}) and the highest being \TODO{$0.8908$ (GAT trained on CiteSeer and adversary trained on Pubmed with 80\% known edges}).

\chapter{Background}
  ...
  \section{Neural Networks}
    ...
    \subsection*{Transductive Setting}
      ...
      The model learns hidden layer representations that encode both local graph structure and features of nodes to perform its task, by aggregating information throughout the graph.
  ...
  \section{Link Stealing Attacks}
    ...
    Link Stealing Attacks are introduced the first time by He et al. \cite{DBLP:journals/corr/abs-2005-02131}.
    ...
    Todo so, the authors queried their target model $f$ on two nodes $u, v$ obtaining two posterior outputs $f(u)$ and $f(v)$.
    Since graph neural networks in general aggregate their information for one node based on the other nodes surrounding it, connected nodes should have more similar vectors, than nodes, that are not linked.
    So they calculated eight different distances between the two output posteriors $f(u)$ and $f(v)$ and use them as input for the attacker model, which then predicts whether the two nodes are connected to each other or not.

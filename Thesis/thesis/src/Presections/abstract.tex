...
\abstract{
  ...
  In our work, we now focus on the inductive setting. 
  Specifically, given a black box access to a graph neural network model that was trained inductively, we are able to predict whether there exists a link between any two nodes of the training graph or not.
  ...
  Depending on datasets and the graph neural network model we achieve up to \TODO{$0.8909$} F1-Score while having an average success rate of \TODO{$0.82$} F1-Score regarding all our attacks.
